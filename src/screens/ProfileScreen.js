  ...
  return (
    <View style={[SS.resRow, archived && { opacity: 0.5 }]}>
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={SS.resTitle} numberOfLines={1}>
          {title}
        </Text>
      ...
      <View style={{ alignItems: "flex-end", gap: 3 }}>
        {!!badge && (
          <View style={[SS.resBadge, danger && SS.resBadgeDanger]}>
            <Text style={[SS.resBadgeTxt, danger && { color: "#ffb3b3" }]}>
              {badge}
            </Text>
          </View>
        )}
...
export default function ProfileScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: prof } = await supabase
        .from("profiles")
        .select("public_name, notifications_enabled, expo_push_token")
        .eq("id", user.id)
        .maybeSingle();
      ...
      setNotifEnabled(
        typeof safe.notifications_enabled === "boolean"
          ? safe.notifications_enabled
          : false
      );
      ...
      try {
        const ids = [
          ...new Set(rows.map((r) => r.item_id).filter(Boolean).map(String)),
        ];
        ...
        if (ids.length) {
          ...
          if (items?.length) {
            ...
            rows = rows.map((r) => ({
              ...r,
              item_title:
                (r.item_id ? map.get(String(r.item_id)) : null) ||
                r.item_title ||
                "Objet",
            }));
          }
      ...
      setPendingIn(
        rows.filter((r) => String(r.owner_id) === uid && r.status === "pending")
      );

      setBorrowing(
        rows
          .filter(
            (r) => String(r.borrower_id) === uid && r.status === "accepted"
          )
          .map((r) => ({
            ...r,
      ...
      setLending(
        rows.filter((r) => String(r.owner_id) === uid && r.status === "accepted")
      );

      setHistory(
        rows
          .filter((r) => DONE_STATUSES.includes(r.status) && !snap.has(String(r.id)))
          .slice(0, 20)
      );
  ...
  const savePseudo = useCallback(async () => {
    ...
    if (!clean || clean.length < 3) {
      Alert.alert("Pseudo", "Au moins 3 caractères.");
      return;
    }
  ...
  const overdueCount = useMemo(
    () => borrowing.filter((r) => r.overdue).length,
    [borrowing]
  );
      ...
      >
          ...
          <View style={SS.urgenceSection}>
            <View style={SS.sectionLabel}>
              <MaterialCommunityIcons
                ...
                color={colors.danger}
              />
              <Text style={[SS.sectionLabelTxt, { color: colors.danger }]}>
                À traiter
              </Text>
            </View>
            ...
            {overdueCount > 0 && (
              <View style={[SS.resRow, SS.resRowDanger]}>
                <MaterialCommunityIcons
                  ...
                  color={colors.danger}
                />
                <Text style={SS.lateText}>
                  {overdueCount === 1
                    ? "1 objet en retard"
                    : `${overdueCount} objets en retard`}
                </Text>
              </View>
            )}
            ...
            {pendingIn.map((r) => (
              <View key={r.id} style={SS.pendingCard}>
                <View style={SS.pendingHead}>
                  ...
                  <Text style={SS.pendingTitle} numberOfLines={1}>
                    {r.item_title || "Objet"}
                  </Text>
        ...
        {(borrowing.length > 0 || lending.length > 0) && (
          <Accordion
            icon="progress-clock"
            title="En cours"
            badge={borrowing.length + lending.length}
            defaultOpen
          >
            {borrowing.length > 0 && (
              <>
            ...
            {lending.length > 0 && (
              <>
                <Text style={[SS.subHead, borrowing.length > 0 && { marginTop: 10 }]}>
                  Mes prêts
                </Text>
        ...
        {(historyActive.length > 0 || archivedList.length > 0) && (
          <Accordion
            icon="history"
            title="Historique"
            badge={historyActive.length + archivedList.length}
            defaultOpen={false}
          >
              ...
              <>
                ...
                {historyActive.map((r) => (
                  <ResRow
                    key={r.id}
                    title={r.item_title}
                    meta={fDate(r.created_at)}
                    badge={fResStatus(r.status)}
                    archivable
        ...
        {calls.length > 0 && (
          ...
          >
            {calls.map((c) => (
              <ResRow
                key={c.id}
                title={c.message || "Onde"}
                meta={fDate(c.created_at)}
                badge={fCallStatus(c.status)}
              />
            ))}
        ...
        <View style={SS.about}>
          ...
          >
            <MaterialCommunityIcons
              ...
              color={colors.subtext}
            />
            <Text style={SS.privacyText}>Confidentialité</Text>
        ...
        <TouchableOpacity onPress={deleteAccount} style={SS.deleteBtn} activeOpacity={0.88}>
          <MaterialCommunityIcons
            ...
            color={colors.danger}
          />
          <Text style={SS.deleteTxt}>Supprimer mon compte</Text>
        </TouchableOpacity>
...
const SS = StyleSheet.create({
  ...
  statChip: {
    ...
    borderWidth: 1,
    borderColor: "rgba(29,255,194,0.18)",
    borderRadius: 999,
